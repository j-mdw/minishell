...
/*   Updated: 2021/02/22 14:46:02 by user42           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	param_expand(t_exp_utils utils, int total_len)
{
	...
	{
		while (*utils.raw_param && *utils.raw_param != '$' && ++i)
			utils.raw_param++;
	}
	if (*utils.raw_param)
	...
	else
	{
		if (!(utils.final_param = malloc(total_len + i + 1)))
			return (NULL); // Error on malloc, how to handle ??
	...
	param_trim(char *raw_param, t_list *local_env)
{
	...
	if (!raw_param)
		return (NULL);
	lit_status_init(&(utils.lit_status));
